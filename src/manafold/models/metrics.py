...
def calibration_metrics(
  *,
  ...
  targets = torch.tensor(
    [
      label_to_idx.get(example.target_label_id, -1)
  ...
  return {
    ...
    "selection": {
      ...
      "excluded_unknown_count": (
        len(split_indexes.get("validation", [])) - len(validation_known_indexes)
      ),
    },
    "uncalibrated": {
...
def annotate_temperature_scaled_confidence(
  ...
  *,
  ...
  if logits is None or not isinstance(temperature, int | float) or temperature <= 0:
    return
...
def _metric_block(predictions: list[dict[str, Any]]) -> dict[str, Any]:
  ...
  top_3_correct = sum(
    1
    for prediction in predictions
    if prediction.get("is_top_3_correct")
  )
  return {
...
def _label_coverage(predictions: list[dict[str, Any]]) -> dict[str, Any]:
  ...
  return {
    ...
    "actual_never_predicted": sorted(set(actual_labels) - set(predicted_labels)),
    "predicted_never_actual": sorted(set(predicted_labels) - set(actual_labels)),
  }
...
def _confidence_summary(predictions: list[dict[str, Any]]) -> dict[str, float | None]:
  scores = sorted(
...
def _abstention_report(
  ...
  if _has_score(split_rows, "temperature_scaled_max_softmax_probability"):
    report["temperature_scaled_max_softmax_probability"] = _score_abstention_report(
      split_rows,
      train_label_support,
...
def _select_abstention_threshold(
  ...
  *,
  ...
  total_known_correct = sum(
    1
    for row in rows
    if not row["is_unknown"] and row["is_correct"]
  )

  rows.sort(key=lambda row: row["unknown_score"], reverse=True)
  max_score = float(rows[0]["unknown_score"])
  min_score = float(rows[-1]["unknown_score"])
  ...
  def maybe_update(
    ...
    *,
    ...
    known_abstained: int,
    known_correct_abstained: int,
  ) -> None:
    nonlocal best_threshold, best_key
    ...
    known_accuracy = (
      known_correct_accepted / known_accepted
      if known_accepted
      else 0.0
    )
    coverage = (known_accepted + (unknown_count - unknown_abstained)) / len(rows)
    selection_score = (unknown_recall + known_acceptance) / 2
    key = (selection_score, known_accuracy, coverage)
  ...
  while index < len(rows):
    ...
    maybe_update(
      ...
      known_correct_abstained=known_correct_abstained,
    )
  ...
  *,
  ...
  return {
    **partition,
    ...
    "fpr_at_95_unknown_recall": _known_false_abstention_rate_at_unknown_recall(
      predictions,
      train_label_support,
      score_key=score_key,
      higher_means_unknown=higher_means_unknown,
    ...
    rows.append(
      {
        ...
        "is_unknown": _actual_label_support(
          prediction,
          train_label_support,
        ) == 0,
