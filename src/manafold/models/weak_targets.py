  ...
  *,
  ...
  result = {
    ...
    "rows_with_train_coverage_count": sum(
      ...
      if int(row["train_rows_affected"]) > 0
    ),
    "rows_with_any_coverage_count": sum(
      1
      for row in preview_rows
      if int(row["total_rows_affected"]) > 0
    ),
...
def _load_dataset_rows(
  ...
  *,
  ...
  labels = {
    ...
    for row in pq.read_table(dataset_path / artifacts["proxy_targets"]).to_pylist()
...
def _edge_index(edges: list[dict[str, Any]]) -> dict[tuple[Any, str, str, str], dict[str, Any]]:
  return {
    (
      edge.get("format"),
      str(edge["label_a_id"]),
...
def _affected_counts(
  ...
  *,
  ...
  by_split: Counter[str] = Counter({split_name: 0 for split_name in SPLIT_ORDER})
  ...
  return [
    {
      ...
      "excluded_because_heldout": (
        scope.get("training_exclusion_reason")
