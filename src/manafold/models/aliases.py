...
from manafold.models.train import MODEL_DEEPSETS_QUANTITY_WEIGHTED_REGULARIZED
...
def run_alias_candidate_scoring(
  ...
  *,
  ...
  model_name: str = MODEL_DEEPSETS_QUANTITY_WEIGHTED_REGULARIZED,
  ...
  if rolling_result is None:
    raise ValueError("rolling_result is required unless --predictions is passed.")
  ...
  if output is not None:
    ...
    _write_jsonl(_weak_label_observations_path(output), weak_label_observations)
  return result
...
def _prediction_backed_alias_candidates(
  *,
  ...
  prediction_confusion = _prediction_confusion_counts_by_split(prediction_rows)
  ...
  for split_name in split_names:
    ...
    for pair_key in pair_keys:
      ...
      profile_a = profiles.get((split_name, label_a), _empty_profile(label_a, split_name))
      profile_b = profiles.get((split_name, label_b), _empty_profile(label_b, split_name))
  ...
  result = {
    ...
    "backfill_report_path": (
      str(_backfill_report_path(output))
      if output is not None
      else None
    ),
...
def _window_alias_candidates(
  ...
  *,
  ...
  for split_name in split_names:
    pair_keys = set(confusion.get(split_name, {})) | set(seed_pairs.get(split_name, ()))
    for pair_key in pair_keys:
...
def _load_deck_records(
  ...
  *,
  ...
  splits = {
    str(row["deck_id"]): row
    for row in pq.read_table(dataset_path / artifacts["split_manifest"]).to_pylist()
  }
  labels = {
  ...
  cards_by_deck: dict[str, dict[str, set[int]]] = defaultdict(lambda: defaultdict(set))
...
def _label_support_by_split(
  ...
  return {
    ...
    for split_name, counter in supports.items()
  }


def _label_profiles(records: list[_DeckRecord]) -> dict[tuple[str, str], _LabelProfile]:
...
def _seed_names(model_result: dict[str, Any]) -> tuple[str, ...]:
  names = [
    _seed_name(path)
    for path in _prediction_paths(model_result)
  ]
...
def _orient_pair_by_support(
  ...
  *,
  ...
  if right_count > left_count:
    return right, left
  if right_count == left_count and display_label_from_id(right) < display_label_from_id(left):
    return right, left
  return left, right
  ...
  *,
  ...
  return {
    ...
    **seed_stability,
    ...
    "date_window": _candidate_date_window(records, split_name, (label_a, label_b)),
  }
...
def _overlap(
  ...
  max_sum = sum(max(adoption_a.get(card, 0.0), adoption_b.get(card, 0.0)) for card in cards)
  min_sum = sum(min(adoption_a.get(card, 0.0), adoption_b.get(card, 0.0)) for card in cards)
  return {
...
def _seed_stability(
  ...
  *,
  ...
  if seed_names:
    counts = [
      int(pair_confusions.get(seed_name, 0))
      for seed_name in seed_names
    ]
  else:
    counts = [
      int(count)
      for _, count in sorted(pair_confusions.items())
    ]
  total = sum(counts)
  ...
  active_count = sum(
    1
    for count in counts
    if count >= min_seed_confusion_count
  )
...
def _pair_summaries(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
  ...
  for key, group in grouped.items():
    ...
    seed_alias_match_any = any(
      bool(row["evidence"].get("seed_alias_match"))
      for row in group
    )
    best = max(
      group,
...
def _orient_summary_pair(
  ...
  if score[right] > score[left]:
    return right, left
  if score[right] == score[left] and display_label_from_id(right) < display_label_from_id(left):
    return right, left
  return left, right
...
def _pair_recommendation(
  ...
  *,
  ...
  if seed_alias_match_any and relation in ("alias_candidate", "same_family_candidate"):
...
def _unknown_review_candidates(
  ...
  *,
  ...
  rows: list[dict[str, Any]] = []
  for row in prediction_rows:
    if row.get("source_label_id") is not None and not row.get("is_low_confidence"):
      continue
...
def _backfill_report(
  ...
  *,
  ...
  return {
    ...
    "low_confidence_rate": (
      len(low_confidence) / len(prediction_rows)
      if prediction_rows
      else 0.0
    ),
...
def _embedding_count(deck_embeddings: Path | None, manifest: dict[str, Any]) -> int | None:
...
def _core_coverage(
  ...
  *,
  ...
  shared = [
    ...
    if minority.adoption_by_zone.get(ZONE_MAIN, {}).get(card, 0.0) >= adoption_threshold
  ]
  return {
    "coverage": f"{len(shared)}/{len(majority_core)}",
    "shared_card_names": [
      card_names.get(card, str(card))
      for card in shared[:25]
    ],
  }
