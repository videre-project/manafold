...
def run_weak_relation_graph(
  ...
  *,
  ...
  _write_jsonl(suggestions_output, suggestions)
  return result
  ...
  *,
  ...
  for (format_code, label_a_id, label_b_id, relation), group in grouped.items():
    ...
    seed_consensus_values = [
      float(row.get("seed_consensus") or 0.0)
      for row in group
    ]
    seed_entropy_values = [
      float(row.get("seed_entropy_normalized") or 0.0)
      for row in group
    ]
    ...
    rows.append({
      ...
      "aggregate_confusion_count": sum(
        int(row.get("confusion_count") or 0)
        for row in group
      ),
      "seed_consensus": {
