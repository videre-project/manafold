...
class PackageFeatureSet:
  def __init__(
    ...
    *,
    ...
    self._activation_override_by_deck_id = (
      activation_override_by_deck_id or {}
    )
    ...
    for feature in self.features:
      ...
      sizes_by_zone.setdefault(feature.zone_idx, set()).add(
        len(feature.card_idxs)
      )
    self._sizes_by_zone = {
  ...
  def activation_indexes(self, example: DeckModelInput) -> tuple[int, ...]:
  ...
  def with_shuffled_activations(
    ...
    *,
    ...
    groups: dict[str, list[DeckModelInput]] = defaultdict(list)
    ...
    for grouped_examples in groups.values():
      ...
      activations = [
        self.activation_indexes(example) for example in grouped_examples
      ]
  ...
  def with_zero_activations(
    ...
    return PackageFeatureSet(
      ...
      activation_override_by_deck_id={
        example.deck_id: () for example in examples
      },
    )
  ...
  def _train_activation_rates(
    ...
    return [count / len(train_examples) for count in counts]

  def summary(self, *, limit: int = 50) -> dict[str, Any]:
    return {
      "count": len(self.features),
      "features": [feature.to_dict() for feature in self.features[:limit]],
    }
    ...
    *,
    ...
    for feature in self.features[:limit]:
      ...
      for split_name in _ordered_diagnostic_splits(examples_by_split):
        split_inputs = examples_by_split.get(split_name, [])
        active_count = split_counts.get(split_name, {}).get(
          feature.package_idx, 0
        )
        row[f"{split_name}_activation_count"] = active_count
        row[f"{split_name}_activation_rate"] = (
          active_count / len(split_inputs) if split_inputs else None
        )
      rows.append(row)
    ...
    *,
    ...
    for split_name, examples in examples_by_split.items():
      split_counts: dict[int, int] = {
        feature.package_idx: 0 for feature in features
      }
      for example in examples:
...
def mine_package_features(
  ...
  *,
  ...
  if scoring not in PACKAGE_SCORING_MODES:
    ...
    raise ValueError(
      f"Unsupported package scoring {scoring!r}; choose one of: {supported}."
    )
  ...
  transactions_by_zone = _transactions_by_zone(
    train_examples, zone_idxs=zone_idxs
  )
...
def mine_support_matched_unscored_package_features(
  ...
  *,
  ...
  reference_keys = {
    (feature.zone_idx, feature.card_idxs) for feature in reference_features
  }
  candidates_by_key: dict[tuple[int, int, str], list[_Candidate]] = defaultdict(
    list
  )
...
def _discriminative_stats(
  ...
  *,
  ...
  float, str | None, int | None, float | None, float | None, float | None
]:
...
def _package_type(
  ...
  *,
  ...
  land_count = sum(1 for card_idx in card_idxs if _is_land(card_vocab, card_idx))
...
def _is_land(card_vocab: tuple[dict[str, Any], ...], card_idx: int) -> bool:
  """Check if a card is a land using its type_line from the card vocabulary."""
  if 0 <= card_idx < len(card_vocab):
    type_line = card_vocab[card_idx].get("type_line")
