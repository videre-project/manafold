...
from manafold.models.classifiers import POOLING_SUM
...
def evaluate_rolling_windows(
  *,
  ...
  output: Path,
  model_names: tuple[str, ...] = (MODEL_ALL,),
  ...
  for window in windows:
    ...
    check_result = verify_dataset(export_summary.output)
    training_output = (
      export_summary.output / "model_runs" / "training_results.json"
    )
    ...
    window_results.append(
      _window_result(
        ...
        check_result={
          ...
          "checked_parquet_files": check_result.checked_parquet_files,
        },
  ...
  result = {
    ...
    "model_names": list(window_results[0]["models"]) if window_results else [],
...
def summarize_rolling_window_results(
  ...
  for model_name in model_names:
    ...
    for metric_name in ROLLING_PRIMARY_METRICS:
      metric_rows[metric_name] = summary_statistics([
...
def _model_window_diagnostics(
  ...
  open_set_count = _number(views.get("open_set_unseen_labels", {}).get("count"))
  ...
  return {
    ...
    "top_labels_by_mass": split_metrics.get("per_label_accuracy_top_20", [])[
      :20
    ],
    "largest_confusion_pairs": split_metrics.get("largest_confusion_pairs", [])[
      :10
    ],
...
def _primary_seed_metric_deltas(
  ...
  model_split = (
    model_result.get("metrics", {}).get("splits", {}).get(primary_split, {})
  )
  pooled_split = (
    pooled_model.get("metrics", {}).get("splits", {}).get(primary_split, {})
  )
  model_views = (
...
def _per_label_accuracy_by_label(
  ...
  return {str(row["label_id"]): row for row in rows if "label_id" in row}
...
def _ranking_by_metric(
  ...
  for window in window_results:
    ...
    values = [
      (model_name, value) for model_name, value in values if value is not None
    ]
    values.sort(
  ...
  return {
    ...
    "mean_rank_by_model": {
      model_name: summary_statistics(ranks)
  ...
  for model_name in model_names:
    ...
    for metric_name in ROLLING_RANKING_METRICS:
      ...
      wins = sum(
        1
        for delta in deltas
        if delta is not None and (delta < 0 if lower_is_better else delta > 0)
      )
      losses = sum(
        1
        for delta in deltas
      ...
      metric_rows[metric_name] = {
        ...
        "losses": losses,
        "ties": ties,
        "delta": summary_statistics(deltas),
      }
