...
from manafold.models.data import TrainingDataset
...
def build_family_card_ranking(
  ...
  *,
  artifact_card_vocab: tuple[dict[str, Any], ...],
  ...
  family_ids = {
    ...
    for family in family_vocab["families"]
  }
  artifact_idx_by_oracle = {
    str(card["oracle_id"]): int(card["card_idx"])
    for card in artifact_card_vocab
  }
  artifact_idx_by_dataset_idx = {
    int(card["card_idx"]): artifact_idx_by_oracle.get(str(card["oracle_id"]))
  ...
  for example in dataset.examples:
    ...
    cards = {
      ...
      for artifact_idx in (artifact_idx_by_dataset_idx.get(token.card_idx),)
  ...
  for family_id in sorted(family_ids):
    ...
    for card_idx, in_family_count in family_card_decks[family_id].items():
      ...
      family_adoption = (
        in_family_count + smoothing * global_adoption
      ) / (family_count + smoothing)
      outside_adoption = (
        outside_card_count + smoothing * global_adoption
