...
def run_weak_relation_state(
  *,
  ...
  observations = read_jsonl(alias_observations)
  ...
  deferred_soft_canonical_evidence = [
    row for row in deferred if row["deferred_training_eligible_if_future"]
  ]
  deferred_review_diagnostics = [
    row for row in deferred if not row["deferred_training_eligible_if_future"]
  ]
  blocked = _blocked(
    graph.get("edges", []),
    preview_index=preview_index,
...
def _review_queue(
  ...
  *,
  ...
  for edge in edges:
    ...
    is_review_worthy = confidence >= high_confidence_threshold or (
      has_mixed_evidence and confidence >= review_confidence_threshold
    )
    ...
    rows.append({
      **_edge_summary(edge),
      ...
      "action": _review_action(
        edge, confidence=confidence, train_rows=train_rows
      ),
    })
  return sorted(
    rows,
...
def _review_action(
  edge: dict[str, Any], *, confidence: float, train_rows: int
) -> str:
...
def _bucket_summaries(
  ...
  return {
    bucket_name: {
      "count": len(rows),
      "by_relation": _count_values(row.get("relation") for row in rows),
  ...
  for group in grouped.values():
    ...
    rows.append({
      ...
      "aggregate_confusion_count": sum(
        int(row.get("aggregate_confusion_count") or 0) for row in group
      ),
      "relations": _count_values(row.get("relation") for row in group),
      "review_reasons": _count_values(
        reason for row in group for reason in row.get("review_reasons", [])
      ),
      ...
      "splits": sorted({
        split_name for row in group for split_name in _row_split_names(row)
      }),
    })
...
def _row_split_names(row: dict[str, Any]) -> list[str]:
  ...
  return [str(split_name) for split_name in row.get("splits", [])]
...
def _row_window_names(row: dict[str, Any]) -> list[str]:
  ...
  return [str(window_name) for window_name in row.get("rolling_windows", [])]
